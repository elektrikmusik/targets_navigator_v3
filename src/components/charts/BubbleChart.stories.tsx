...
const meta: Meta<typeof BubbleChart> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'A bubble chart component using Nivo ScatterPlot to visualize companies data with Strategic Fit (X-axis), Ability to Execute (Y-axis), and Overall Score (bubble size). Based on [Nivo ScatterPlot](https://nivo.rocks/scatterplot/).'
      }
    }
  },
  argTypes: {
    ...
    // Node configuration
    nodeSize: {
      description: 'Base size multiplier for nodes (actual size scales with overallScore)',
    ...
    motionConfig: {
      ...
      control: { type: 'select', options: ['gentle', 'wobbly', 'stiff', 'slow', 'molasses', 'default'] }
    },
    // Interactivity
    onNodeClick: {
    ...
    blendMode: {
      ...
      control: { type: 'select', options: ['normal', 'multiply', 'screen', 'overlay', 'darken', 'lighten', 'color-dodge', 'color-burn', 'hard-light', 'soft-light', 'difference', 'exclusion'] }
    },
    // Theme configuration
    theme: {
