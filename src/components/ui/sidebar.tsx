...
export function Sidebar({ open, setOpen, children }: SidebarProps) {
  return (
    <>
      ...
      <div
        className={cn(
          'relative flex h-full flex-col border-r border-neutral-200 bg-white transition-all duration-300 ease-in-out dark:border-neutral-700 dark:bg-neutral-800',
          open ? 'w-64' : 'w-16',
          'md:relative md:h-full',
          'fixed left-0 top-0 z-50 h-screen md:relative md:h-full'
        )}
      >
        {children}
      </div>
...
export function SidebarLink({ link, open = true }: SidebarLinkProps) {
  return (
    <Link
      to={link.href}
